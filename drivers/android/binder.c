...
static void binder_free_proc(struct binder_proc *proc)
{
	...
	kmem_cache_free(binder_proc_wrap_pool, eproc);
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	eproc = kmem_cache_zalloc(binder_proc_wrap_pool, GFP_KERNEL);
	proc = &eproc->proc;
	if (proc == NULL)
		return -ENOMEM;
...
static int __init binder_create_pools(void)
{
       ...
       binder_proc_wrap_pool = KMEM_CACHE(binder_proc_ext, SLAB_HWCACHE_ALIGN);
